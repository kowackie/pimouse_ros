...
class LightsensorsTest(unittest.TestCase):
    def setUp(self):
        self.count = 0
    ...
    def test_change_parameter(self):
        ...
        c_prev = self.count
        time.sleep(3)
        self.assertTrue(self.count < f_prev + 4, "freq does not change")
        self.assertFalse(self.count == f_prev, "subscriber is stopped")

if __name__ == '__main__':
    time.sleep(3)
    rospy.init_node('travis_test_lightsensors')
    rostest.rosrun('pimouse_ros', 'travis_test_lightsensors', LightsensorsTest)
