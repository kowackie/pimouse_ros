...
class Motor(object):
    ...
    def set_raw_freq(self, left_hz, right_hz):
        ...
        try:
            ...
            rospy.logerr("connot write to rtmotor_raw_*")
    ...
    def callback_tm(self, message):
        ...
        try:
            with open(dev, 'w') as f:
                f.write(f"{message.left_hz} {message.right_hz} {message.duration_ms}\n")
        except:
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if m.using_cmd_vel and rospy.Time.now().to_sec() - m.last_time.to_sec() >= 1.:
            m.set_raw_freq(0, 0)
